...
class RadializationKernel(GPModule):
	...
	def forward(self, input1, input2=None):
		...
		if input2 is None:
			...
			term1 = torch.exp(self.log_kernel_amp()).data.squeeze()
			term2 = term1.item() if len(term1.size()) == 0 else term1[0]
			stabilizer = Variable(torch.diag(input1.data.new(input1.size(0)).fill_(1e-6 * term2)))
