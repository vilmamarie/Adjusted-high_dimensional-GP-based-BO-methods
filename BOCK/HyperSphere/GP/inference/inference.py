...
class Inference(nn.Module):
	...
	def init_parameters(self):
		train_tensor = torch.std(self.train_y).detach()
		amp = train_tensor.item() if len(train_tensor.shape) == 0 else train_tensor[0]
	...
	def cholesky_update(self, hyper):
		...
		while True:
			try:
				self.cholesky = torch.cholesky(self.gram_mat + Variable(eye_mat) * chol_jitter, False)
				torch.solve(self.gram_mat[:, :1], self.cholesky)
				break
			except RuntimeError:
	...
	def predict(self, pred_x, hyper=None, in_optimization=False):
		...
		chol_solver = torch.solve(torch.cat([k_pred_train.t(), self.mean_vec], 1), self.cholesky)[0]
	...
	def negative_log_likelihood(self, hyper=None):
		if hyper is not None:
			param_original = self.model.param_to_vec()
			self.cholesky_update(hyper)
		mean_vec_sol = torch.solve(self.mean_vec, self.cholesky)[0]
	...
	def sampling(self, n_sample=10, n_burnin=100, n_thin=10):
		...
		def logp(hyper):
			...
			likelihood = -self.negative_log_likelihood(hyper_tensor).data.squeeze()
			likelihood = likelihood.item() if len(likelihood.size()) == 0 else likelihood[0]
