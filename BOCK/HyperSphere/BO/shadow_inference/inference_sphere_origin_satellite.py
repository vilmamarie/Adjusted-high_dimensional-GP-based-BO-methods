...
class ShadowInference(Inference):
	def __init__(self, train_data, model):
		...
		origin_mask = (torch.sum(self.train_x ** 2, 1) == 0).type(torch.uint8)
		n_origin = torch.sum(origin_mask).item()#.data[0]
		assert n_origin == 1
	...
	def cholesky_update(self, hyper):
		...
		while True:
			try:
				self.cholesky_nonorigin = torch.cholesky(gram_mat_nonorigin + Variable(eye_mat) * chol_jitter, False)
				torch.solve(gram_mat_nonorigin[:, :1], self.cholesky_nonorigin)
				break
			except RuntimeError:
	...
	def predict(self, pred_x, hyper=None, in_optimization=False):
		...
		kernel_max = self.model.kernel.forward_on_identical().item() #.data[0]
		...
		chol_solver = torch.solve(chol_B, self.cholesky_nonorigin)[0]
...
if __name__ == '__main__':
	...
	if ndim == 2:
		...
		print(torch.min(pred_var_shadow).item(), torch.max(pred_var_shadow).item())
		print('l2 distance', torch.dist(pred_var_shadow, pred_var_input_map).item())
		print('l-inf distance', torch.max(torch.abs(pred_var_shadow - pred_var_input_map)).item())
