...
class ShadowInference(Inference):
	...
	def predict(self, pred_x, hyper=None, stability_check=False, in_optimization=False):
		...
		chol_solver = torch.solve(torch.cat([K_train_pre, self.mean_vec, K_train_sat], 1), self.cholesky)[0]
