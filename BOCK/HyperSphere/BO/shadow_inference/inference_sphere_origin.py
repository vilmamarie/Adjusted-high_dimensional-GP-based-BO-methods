...
class ShadowInference(Inference):
	def __init__(self, train_data, model):
		...
		ind_origin = torch.sort(origin_mask.int(), 0, descending=True)[1]
	...
	def cholesky_update(self, hyper):
		...
		eye_mat = torch.eye(self.gram_mat_nonorigin.size(0)).type_as(self.gram_mat_nonorigin)
		chol_jitter = 0
		while True:
			try:
				self.cholesky_nonorigin = torch.cholesky(self.gram_mat_nonorigin + Variable(eye_mat) * chol_jitter, False)
				torch.solve(self.gram_mat_nonorigin[:, :1], self.cholesky_nonorigin)
				break
			except RuntimeError:
	...
	def predict(self, pred_x, hyper=None, in_optimization=False):
		...
		kernel_max = self.model.kernel.forward_on_identical().item()
		n_pred, n_dim = pred_x.size()
		pred_x_radius = torch.sqrt(torch.sum(torch.pow(pred_x.clone(), 2), 1, keepdim=True))

		assert (pred_x_radius.clone() > 0).all()
		pred_x_sphere = pred_x.clone() / pred_x_radius
		...
		K_non_pre = self.model.kernel(self.train_x_nonorigin, pred_x.clone())
		K_ori_pre_diag = self.model.kernel.radius_kernel(pred_x_radius, one_radius * 0)

		chol_solver = torch.solve(torch.cat([K_non_ori, K_non_pre, self.mean_vec.index_select(0, self.ind_nonorigin)], 1), self.cholesky_nonorigin)[0]
		chol_solver_q = chol_solver[:, :n_pred]
		chol_solver_k = chol_solver[:, n_pred:n_pred * 2]
		chol_solver_y = chol_solver[:, n_pred * 2:n_pred * 2 + 1]

		sol_p_sqr = kernel_max + self.model.likelihood(pred_x).view(-1, 1) + self.jitter - (chol_solver_q ** 2).sum(
			0).view(-1, 1)

		if not (sol_p_sqr.clone().detach() >= 0).all():
			if not in_optimization:
				neg_mask = sol_p_sqr.clone().detach() < 0
				neg_val = sol_p_sqr.clone().detach()[neg_mask]
		...
		sol_y_bar = (self.mean_vec.index_select(0, self.ind_origin) - torch.mm(chol_solver_q.t(),
																			   chol_solver_y)) / sol_p

		pred_mean = torch.mm(chol_solver_k.t(), chol_solver_y) + sol_k_bar * sol_y_bar + self.model.mean(pred_x.clone())
		pred_var = self.model.kernel.forward_on_identical() - (chol_solver_k ** 2).sum(0).view(-1, 1) - sol_k_bar ** 2

		if not (pred_var.detach() >= 0).all():
			if not in_optimization:
				neg_mask = pred_var.clone().detach() < 0
				neg_val = pred_var.clone().detach()[neg_mask]
				min_neg_val = torch.min(neg_val)
				max_neg_val = torch.max(neg_val)
				kernel_max = self.model.kernel.forward_on_identical().item()
		...
		numerically_stable = (pred_var.detach() >= 0).all()
		zero_pred_var = (pred_var.detach() <= 0).all()

		return pred_mean, pred_var.clamp(min=1e-12), numerically_stable, zero_pred_var

	def predict2(self, pred_x, hyper=None, in_optimization=False):
		if hyper is not None:
			param_original = self.model.param_to_vec()
			self.cholesky_update(hyper)
		kernel_max = self.model.kernel.forward_on_identical().item()
	...
	def negative_log_likelihood(self, hyper=None):
		...
		chol_solver = torch.solve(torch.cat([self.mean_vec.index_select(0, self.ind_nonorigin), K_non_ori_rel], 1), self.cholesky_nonorigin)[0]
