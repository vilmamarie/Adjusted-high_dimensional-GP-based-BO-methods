...
from HyperSphere.BO.acquisition.acquisition_functions import probability_improvement
from HyperSphere.BO.utils.sobol import sobol_generate

N_SPREAD = 200 # Number of sobol sequence points as candidates for good initial points
...
N_AVAILABLE_CORE = 4 # When there is this many available cpu cores new optimization is started
...
def suggest(x0, reference, inferences, acquisition_function=expected_improvement, bounds=None, pool=None,
            global_constants_dir=None):
    if global_constants_dir is not None:
        sys.path.append(global_constants_dir)
        from global_ import *
    ...
    if pool is not None:
        ...
        while process_started.count(False) > 0:
            ...
            while len(p_queue) < N_AVAILABLE_CORE and process_index < n_init:
                p = multiprocessing.Process(target=optimize, args=(
                    max_step, x0_detached[process_index], reference, inferences, acquisition_function, bounds, q, e))
                p.start()
                p_queue.append(p)
            ...
            for _ in p_queue:
                optimum_q = q.get()
                local_optima.append(optimum_q[0])
                optima_value.append(optimum_q[1])
...
def suggest_original(x0, reference, inferences, acquisition_function=expected_improvement, bounds=None, pool=None, global_constants_dir=None):
    if global_constants_dir is not None:
        sys.path.append(global_constants_dir)
        from global_ import *
    ...
    if pool is not None:
        ...
        while process_started.count(False) > 0:
            ...
            if run_more:
                p = multiprocessing.Process(target=optimize, args=(max_step, x0_detached[process_index], reference, inferences, acquisition_function, bounds, q, e))
                # results.append(pool.apply_async(optimize, args=(
                #			max_step, x0_detached[process_index], reference, inferences, acquisition_function, bounds)))
                p.start()
                results.append(p)
                process_started[process_index] = True
                process_running[process_index] = True
...
def optimize(max_step, x0, reference, inferences, acquisition_function=expected_improvement, bounds=None, q=None, e=None):
    if bounds is not None:
        if not hasattr(bounds, '__call__'):
            def out_of_bounds(x_input):
                return (x_input < bounds[0]).any() or (x_input > bounds[1]).any()
        else:
    ...
    for s in range(max_step):
        ...
        curr_loss = loss.clone().detach().squeeze().item()
        #print(s,": ", curr_loss)
        ...
        if (x.grad.clone() != x.grad.clone()).any() or (ftol < 1e-9):
            break
        prev_x = x.clone()
        prev_loss = curr_loss
        optimizer.step()
    ...
    #print("-"*100)
    optimum_loc = x.clone()
    optimum_value = -acquisition(x, reference=reference, inferences=inferences, acquisition_function=acquisition_function, in_optimization=True)[0].clone().detach().squeeze().item()

    if q is not None:
        q.put([optimum_loc.detach(), optimum_value])
        if e is not None:
            e.wait(7200)
    else:
...
def optimization_candidates(input, output, lower_bnd, upper_bnd, global_constants_dir=None):
    if global_constants_dir is not None:
        sys.path.append(global_constants_dir)
        from global_ import *
    ...
    x0_spray = input.data[min_ind].view(1, -1).repeat(N_SPRAY, 1) + input.data.new(N_SPRAY, ndim).normal_() * 0.1 * (upper_bnd - lower_bnd)
    if hasattr(lower_bnd, 'size'):
        for bnd_idx in range(lower_bnd.size()[0]-1):
            x0_spray[x0_spray[:, bnd_idx] < lower_bnd[bnd_idx], bnd_idx] = 2 * lower_bnd[bnd_idx] - x0_spray[x0_spray[:, bnd_idx] < lower_bnd[bnd_idx], bnd_idx]
        # x0_spray[x0_spray < lower_bnd] = 2 * lower_bnd.view(1, -1).repeat(x0_spray[x0_spray < lower_bnd].size()[0], 1) - x0_spray[x0_spray < lower_bnd]
    else:
        x0_spray[x0_spray < lower_bnd] = 2 * lower_bnd - x0_spray[x0_spray < lower_bnd]

    if hasattr(upper_bnd, 'size'):
        for bnd_idx in range(upper_bnd.size()[0]-1):
            x0_spray[x0_spray[:, bnd_idx] > upper_bnd[bnd_idx], bnd_idx] = 2 * upper_bnd[bnd_idx] - x0_spray[x0_spray[:, bnd_idx] > upper_bnd[bnd_idx], bnd_idx]
    ...
    x0 = torch.cat([input.detach(), x0_spray, x0_spread], 0)

    nonzero_radius_mask = torch.sum(x0 ** 2, 1) > 0
    nonzero_radius_ind = torch.sort(nonzero_radius_mask.int(), 0, descending=True)[1][:torch.sum(nonzero_radius_mask)]
    x0 = x0.index_select(0, nonzero_radius_ind)

    return Variable(x0)


def optimization_init_points(candidates, reference, inferences, acquisition_function=expected_improvement, global_constants_dir=None):
    if global_constants_dir is not None:
        sys.path.append(global_constants_dir)
        from global_ import *
    ...
    acq_value = acq_value.clone().detach()
    nonnan_ind = acq_value == acq_value
    acq_value = acq_value[nonnan_ind]
    init_points = candidates.clone().detach()[nonnan_ind.view(-1, 1).repeat(1, ndim)].view(-1, ndim)
