...
def REMBO_separate(start_rep=1, stop_rep=50, test_func='Rosenbrock', total_itr=100, low_dim=2,
                   ...
                   kern_inp_type='Y', noise_var=0, range_path=None, initial_path=None, output_path=None, **kwargs):
    ...
    result_f_s_true = np.empty((0, initial_n + total_itr, 1))

    param_ranges = None
    if range_path is not None:
        with open(range_path, 'rb') as pickle_file:
            param_ranges = pickle.load(pickle_file).astype(np.float64)

    s = None
    if initial_path is not None:
        with open(initial_path, 'rb') as pickle_file:
            s = pickle.load(pickle_file).astype(np.float64)

    if box_size is None:
        box_size = np.sqrt(low_dim)

    all_A = np.random.normal(0, 1, [stop_rep, low_dim, high_dim]).astype(np.float64)
    ...
    for i in range(start_rep - 1, stop_rep):
        ...
        # Running different algorithms to solve Hartmann6 function
        temp_result, temp_elapsed, temp_s, temp_f_s, temp_f_s_true, temp_high_s = REMBO.RunRembo(low_dim=low_dim, high_dim=high_dim, initial_n=initial_n,
                                                                                     total_itr=total_itr, func_type=test_func, A_input=all_A[i],
                                                                                     s=s, kern_inp_type=kern_inp_type, matrix_type='simple',
                                                                                     hyper_opt_interval=opt_interval, ARD=ARD, box_size=box_size,
                                                                                     noise_var=noise_var, param_ranges=param_ranges, output_path=output_path, **kwargs)
        ...
        result_f_s_true = np.append(result_f_s_true, [temp_f_s_true], axis=0)
    ...
    if output_path is not None:
        ...
        pickle.dump([result_obj, result_s, result_high_s, result_f_s, result_f_s_true, elapsed], fileObject)
        fileObject.close()
...
def count_sketch_BO_experiments(start_rep=1, stop_rep=50, test_func='Rosenbrock', total_itr=100,
                                ...
                                noise_var=0, range_path=None, initial_path=None, output_path=None, **kwargs):
    ...
    if range_path is not None:
        with open(range_path, 'rb') as pickle_file:
            param_ranges=pickle.load(pickle_file)
    ...
    for i in range(start_rep - 1, stop_rep):
        ...
        temp_result, temp_elapsed, temp_s, temp_f_s, temp_f_s_true, temp_high_s = count_sketch.RunMain(low_dim=low_dim, high_dim=high_dim, initial_n=initial_n,
                                                                              total_itr=total_itr, func_type=test_func, s=s, ARD=ARD,
                                                                              box_size=box_size, noise_var=noise_var, param_ranges=param_ranges, output_path=output_path,**kwargs)
...
if __name__=='__main__':
    ...
    range_path = sys.argv[10]
    output_path = sys.argv[11]
    initial_path = sys.argv[12]
    random_seed = sys.argv[13]
    acq_func = sys.argv[14]

    kern_type = "Y"
    if len(sys.argv) > 15:
        kern_type = sys.argv[15]
    ...
    if random_seed != "NULL":
        seed(a=int(random_seed), version=2)

    if acq_func == "NULL":
        acq_func = "EI"
    acq_func = getattr(REMBO, acq_func, None)

    if not callable(acq_func):
        NotImplementedError('The requested acquisition function is not valid!')

    if sys.argv[1] == 'REMBO':
        REMBO_separate(start_rep=start_rep, stop_rep=stop_rep, test_func=test_func, total_itr=total_iter,
                       low_dim=low_dim, high_dim=high_dim, initial_n=initial_n, ARD=True, kern_inp_type=kern_type,
                       noise_var=variance, range_path=range_path, initial_path=initial_path, output_path=output_path, afn=acq_func)
    ...
    elif sys.argv[1] == 'REMBO_X':
        REMBO_separate(start_rep=start_rep, stop_rep=stop_rep, test_func=test_func, total_itr=total_iter,
                       low_dim=low_dim, high_dim=high_dim, initial_n=initial_n, ARD=True, kern_inp_type="X",
                       noise_var=variance, range_path=range_path, initial_path=initial_path, output_path=output_path,
                       afn=acq_func)
    elif sys.argv[1] == 'REMBO_Y':
        REMBO_separate(start_rep=start_rep, stop_rep=stop_rep, test_func=test_func, total_itr=total_iter,
                       low_dim=low_dim, high_dim=high_dim, initial_n=initial_n, ARD=True, kern_inp_type="Y",
                       noise_var=variance, range_path=range_path, initial_path=initial_path, output_path=output_path,
                       afn=acq_func)
    elif sys.argv[1] == 'REMBO_psi':
        REMBO_separate(start_rep=start_rep, stop_rep=stop_rep, test_func=test_func, total_itr=total_iter,
                       low_dim=low_dim, high_dim=high_dim, initial_n=initial_n, ARD=True, kern_inp_type="psi",
                       noise_var=variance, range_path=range_path, initial_path=initial_path, output_path=output_path,
                       afn=acq_func)
    elif sys.argv[1] == 'HeSBO':
        count_sketch_BO_experiments(start_rep=start_rep, stop_rep=stop_rep, test_func=test_func, total_itr=total_iter, low_dim=low_dim, high_dim=high_dim, initial_n=initial_n, ARD=True, box_size=1, noise_var=variance, range_path=range_path, initial_path=initial_path, output_path=output_path, afn=acq_func)